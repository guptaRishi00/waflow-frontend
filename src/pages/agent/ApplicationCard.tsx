...
const ApplicationCard = ({ app, selectedApp, setSelectedApp, applications }: {
  app: any;
  selectedApp: any;
      ...
      <div className="mt-2">
        <div className="w-full bg-gray-200 rounded-full h-1.5">
          <div
            ...
            style={{
              width: `${
                ((app.steps?.filter((step: any) => step.status === 'Approved').length || 0) /
                  (app.steps?.length || 1)) *
                100
              }%`
            }}
        ...
        <span className="text-xs text-muted-foreground">
          Step {(app.steps?.filter((step: any) => step.status === 'Approved').length || 0)}/{app.steps?.length || 0}
        </span>
      </div>
    </div>
  );
