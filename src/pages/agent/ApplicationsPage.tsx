...
export const ApplicationsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCustomerProfileAndDocuments = async () => {
      if (!demo || !demo[0] || !demo[0].customer || !demo[0].customer._id) return;
      ...
      try {
        ...
        const customerProfileRes = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/api/user/customer/profile/${customerId}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      ...
      console.log('Fetching documents for customer:', selectedApp.customer._id, 'and application:', selectedApp._id);
      try {
        ...
        console.log('API response for customerDocuments:', customerRes.data.data);
      } catch (err) {
  ...
  const handleStepAction = async (stepIndex: number, action: 'approve' | 'reject') => {
    ...
    try {
      const response = await axios.patch(
        `${import.meta.env.VITE_BASE_URL}/api/application/step/${selectedApp._id}`,
        {
      ...
      toast({
        ...
        description: `Step "${step.stepName}" has been ${action === 'approve' ? 'approved' : 'rejected'}.`,
      });
    } catch (err) {
      toast({
        title: 'Error',
        ...
        {applications.map((app, index) => (
          <div key={app._id ?? index} className="mb-2">
            <div className="text-xs text-muted-foreground mb-1">
              <strong>Customer ID:</strong> {app.customer?._id || 'N/A'} | <strong>Email:</strong> {app.customer?.email || 'N/A'}
            </div>
            <ApplicationCard
              app={app}
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    {selectedApp?._id} • {selectedApp?.customer?.email} • Created {selectedApp ? new Date(selectedApp.createdAt).toLocaleDateString() : ''}
                  </CardDescription>
                </div>
                <div className="flex items-center space-x-2">
                  ...
                  <Button size="sm" variant="outline" asChild>
                    <Link to={selectedApp ? `/agent/customers/${selectedApp.customer?._id}` : '#'}>
                      <Eye className="h-4 w-4 mr-1" />
                      View Customer
                    </Link>
                  </Button>
                  <Button size="sm" variant="outline" asChild>
                    <Link to={selectedApp ? `/agent/chat?application=${selectedApp._id}` : '#'}>
          ...
          <Card>
            ...
            <CardContent>
              <ProgressTracker
                ...
                currentStep={(selectedApp?.steps?.filter(step => step.status === 'Approved').length || 0)}
                onStepAction={handleStepAction}
              />
              {stepActionLoading && <p className="text-xs text-muted-foreground mt-2">Updating step...</p>}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>All documents uploaded by the customer or for this application</CardDescription>
            </CardHeader>
            <CardContent>
              <CustomerDocuments
                selectedApp={selectedApp}
