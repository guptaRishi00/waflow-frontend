...
interface CustomerDocumentsProps {
  ...
  visaDocuments?: any[]; // array of doc._id or doc.documentName
}
...
const CustomerDocuments: React.FC<CustomerDocumentsProps> = ({
                    ...
                    <div className="flex flex-col gap-2 ml-2">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          onClick={() =>
                            handleStatusUpdate(doc._id, "Rejected", rejectNote)
                          }
                        >
                          {actionLoading === doc._id + "Rejected"
                            ? "Rejecting..."
                ...
                <div className="flex flex-col gap-2 ml-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={
                        actionLoading === doc._id + "note" || !newNote.trim()
                      }
  ...
  // Filter out visa documents from both lists
  const filteredCustomerDocuments = customerDocuments.filter(
    (doc) =>
      !visaDocuments.some(
        (v) => v._id === doc._id || v.documentName === doc.documentName
      )
  );
  const filteredApplicationDocuments = applicationDocuments.filter(
    (doc) =>
      !visaDocuments.some(
        (v) => v._id === doc._id || v.documentName === doc.documentName
      )
        ...
        <div className="space-y-8">
          {filteredCustomerDocuments.length > 0 &&
            renderDocumentList(
              ...
              "Customer-linked Documents"
            )}
          {filteredApplicationDocuments.length > 0 &&
            renderDocumentList(
              filteredApplicationDocuments,
